	...
	tcs := []struct {
	...
	}{
		{
			...
			&pexels.CollectionMediaParams{
				...
				CollectionParams: pexels.CollectionParams{
					Page:    1,
					PerPage: 3,
				},
			},
...
func TestFailedGetCollection(t *testing.T) {
	...
	case err.Error() != "BIG OOF dawg, looks like we found an error":
		t.Error("expected error does match return error")
	}
	if !cmp.Equal(zero, pexels.MediaResponse{}) {
